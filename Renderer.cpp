...
void Renderer::SetGraphicsRootDescriptorTable(ID3D12GraphicsCommandList * pd3dCommandList, UINT nRootParameter)
{
	pd3dCommandList->SetGraphicsRootDescriptorTable(nRootParameter, m_d3dCbvGPUDescriptorStartHandle);
}
...
D3D12_SHADER_BYTECODE Renderer::CreateVertexShader(ID3DBlob **ppd3dShaderBlob)
{
...
D3D12_SHADER_BYTECODE Renderer::CreatePixelShader(ID3DBlob **ppd3dShaderBlob)
{
	D3D12_SHADER_BYTECODE d3dShaderByteCode;
	d3dShaderByteCode.BytecodeLength = 0;
	d3dShaderByteCode.pShaderBytecode = NULL;
...
D3D12_SHADER_BYTECODE Renderer::CompileShaderFromFile(WCHAR *pszFileName, LPCSTR pszShaderName, LPCSTR pszShaderProfile, ID3DBlob **ppd3dShaderBlob)
{
...
D3D12_INPUT_LAYOUT_DESC Renderer::CreateInputLayout()
{
...
D3D12_RASTERIZER_DESC Renderer::CreateRasterizerState()
{
...
D3D12_DEPTH_STENCIL_DESC Renderer::CreateDepthStencilState()
{
...
D3D12_BLEND_DESC Renderer::CreateBlendState()
{
...
void Renderer::CreateCbvAndSrvDescriptorHeaps(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList
	, int nConstantBufferViews, int nShaderResourceViews)
{
	...
	HRESULT hResult = pd3dDevice->CreateDescriptorHeap(&d3dDescriptorHeapDesc, __uuidof(ID3D12DescriptorHeap), &m_d3dDescriptorHeap);

	m_d3dCbvCPUDescriptorStartHandle = m_d3dDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
	m_d3dCbvGPUDescriptorStartHandle = m_d3dDescriptorHeap->GetGPUDescriptorHandleForHeapStart();
...
void Renderer::CreateShaderResourceViews(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, Texture *pTexture, UINT nRootParameterStartIndex, bool bAutoIncrement)
{
	if (pTexture == nullptr)
		return;
...
void Renderer::OnPrepareForRender(ID3D12GraphicsCommandList * pd3dCommandList)
{
...
D3D12_INPUT_LAYOUT_DESC DiffuseRenderer::CreateInputLayout()
{
	...
	pd3dInputElementDescs[2] = { "UV", 0, DXGI_FORMAT_R32G32_FLOAT, 0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[3] = { "CLUSTER", 0, DXGI_FORMAT_R32G32B32A32_UINT, 0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[4] = { "WEIGHT", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
...
D3D12_SHADER_BYTECODE DiffuseRenderer::CreateVertexShader(ID3DBlob **ppd3dShaderBlob)
{
	return(CompileShaderFromFile(L"Default.hlsl", "VS", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE DiffuseRenderer::CreatePixelShader(ID3DBlob **ppd3dShaderBlob)
{
	return (CompileShaderFromFile(L"Default.hlsl", "PS", "ps_5_1", ppd3dShaderBlob));
}